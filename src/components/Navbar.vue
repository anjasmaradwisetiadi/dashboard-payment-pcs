<template>
    <div id="Navbar">
        ...
        <transition name="fade" v-if="nameRoutePath === 'notification'">
                <div class="flex justify-start w-screen flex-row title text-center pb-3 pt-2 border-b-2 mb-8 px-4 border-t z-10 fixed bg-white shadow-md">
                    <div class="mr-3 cursor-pointer flex items-center" @click="goBack()">
                        <span class="material-icons" style="font-size: 24px;">
                            arrow_back
                        </span>
                    </div>
                    <h3 class="text-2xl text-red-primary font-semibold ">Notification</h3>
                </div>
        </transition>
    </div>
</template>
